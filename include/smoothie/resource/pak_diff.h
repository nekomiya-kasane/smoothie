...
public:
    ...
    [[nodiscard]] static auto create(const pak_reader& old_pak,
                                     const pak_reader& new_pak)
...
public:
    ...
    [[nodiscard]] static auto apply(const pak_reader& old_pak,
                                    std::span<const std::byte> patch_data,
                                    const std::filesystem::path& output)
        -> diagnostic_result<void>;

    /// @brief Parse and validate a patch header without applying.
    [[nodiscard]] static auto validate(std::span<const std::byte> patch_data)
        -> diagnostic_result<patch_header>;
};
