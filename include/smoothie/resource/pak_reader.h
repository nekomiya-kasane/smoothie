...
public:
    ...
    [[nodiscard]] static auto open(std::span<const std::byte> buffer)
        -> diagnostic_result<pak_reader>;
    ...
    [[nodiscard]] auto data_of(const entry_descriptor& entry) const noexcept
        -> result<std::span<const std::byte>>;

    /// Get resource data with transparent decompression.
    [[nodiscard]] auto data_of_view(const entry_descriptor& entry) const
