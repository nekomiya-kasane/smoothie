...
inline constexpr uint32_t lpak_version = (static_cast<uint32_t>(lpak_version_major) << 16) | lpak_version_minor;
...
[[nodiscard]] constexpr auto operator|(entry_flags a, entry_flags b) noexcept -> entry_flags {
    return static_cast<entry_flags>(static_cast<uint16_t>(a) | static_cast<uint16_t>(b));
}
