...
public:
    ...
    [[nodiscard]] auto load(uint64_t semantic_hash, uint32_t type_id)
        -> result<loaded_asset>;

    /// Load a resource by semantic hash and type name.
    [[nodiscard]] auto load(uint64_t semantic_hash, std::string_view type_name)
        -> result<loaded_asset>;

    /// Asynchronously load a resource.
    [[nodiscard]] auto load_async(uint64_t semantic_hash, uint32_t type_id)
    ...
    [[nodiscard]] auto dependencies(uint64_t semantic_hash) const
