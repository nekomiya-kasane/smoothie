...
#define SMOOTHIE_C_API_VERSION \
    ((SMOOTHIE_C_API_VERSION_MAJOR << 16) | SMOOTHIE_C_API_VERSION_MINOR)
...
SMOOTHIE_CAPI smoothie_error_code smoothie_vfs_mount(
    smoothie_vfs* vfs,
    const char* name,
    const char* path,
    int priority);
...
SMOOTHIE_CAPI smoothie_error_code smoothie_vfs_mount_memory(
    smoothie_vfs* vfs,
    const char* name,
    const void* data,
    size_t data_size,
...
SMOOTHIE_CAPI smoothie_error_code smoothie_vfs_unmount(
    smoothie_vfs* vfs,
    const char* name);
 ...
 */
SMOOTHIE_CAPI smoothie_error_code smoothie_vfs_get(
    const smoothie_vfs* vfs,
    uint64_t semantic_hash,
    const void** out_data,
    size_t* out_size);

/** Check if a resource exists. Returns 1 if found, 0 otherwise. */
SMOOTHIE_CAPI int smoothie_vfs_contains(
    const smoothie_vfs* vfs,
    uint64_t semantic_hash);
...
SMOOTHIE_CAPI smoothie_error_code smoothie_pak_writer_add(
    smoothie_pak_writer* writer,
    const char* path,
    smoothie_resource_type type,
    const void* data,
    size_t data_size);

/** Set the default compression mode for subsequently added entries. */
SMOOTHIE_CAPI void smoothie_pak_writer_set_compression(
    smoothie_pak_writer* writer,
    smoothie_compression_mode mode);

/** Write the .lpak file to disk. */
SMOOTHIE_CAPI smoothie_error_code smoothie_pak_writer_write(
    const smoothie_pak_writer* writer,
