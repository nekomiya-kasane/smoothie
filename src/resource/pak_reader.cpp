...
auto pak_reader::open(std::span<const std::byte> buffer) -> diagnostic_result<pak_reader> {
    if (buffer.size() < sizeof(file_header)) {
        return std::unexpected(error{error_code::corrupted, std::format("buffer too small: {} bytes, need at least {}",
                                                                        buffer.size(), sizeof(file_header))});
    }
    ...
    if (file_major != lpak_version_major) {
        return std::unexpected(
            error{error_code::version_mismatch,
                  std::format("incompatible major version: file={}, expected={}", file_major, lpak_version_major)});
    }
    if (file_minor > lpak_version_minor) {
        return std::unexpected(error{error_code::version_mismatch, std::format("file minor version {} > current {}",
                                                                               file_minor, lpak_version_minor)});
    }
    ...
    if (expected_checksum != reader.header_->header_checksum) {
        return std::unexpected(
            error{error_code::corrupted, std::format("header checksum mismatch: expected={:#018x}, got={:#018x}",
    ...
    if (index_size / sizeof(entry_descriptor) != reader.header_->entry_count || index_offset > buffer.size() ||
        index_size > buffer.size() - index_offset) {
    ...
    reader.index_ = std::span<const entry_descriptor>(
        reinterpret_cast<const entry_descriptor *>(buffer.data() + index_offset), reader.header_->entry_count);
    ...
    const auto data_end =
        (reader.header_->string_table_offset > 0) ? reader.header_->string_table_offset : buffer.size();
...
auto pak_reader::data_of(const entry_descriptor &entry) const noexcept -> result<std::span<const std::byte>> {
...
auto pak_reader::data_of_view(const entry_descriptor &entry) const -> diagnostic_result<resource_view> {
    const size_t end = static_cast<size_t>(entry.data_offset) + entry.data_size;
    if (end > data_payload_.size()) {
