...
auto pak_writer::write(const std::filesystem::path &output) const -> diagnostic_result<void> {
    ...
    std::sort(index.begin(), index.end(),
              [](const entry_descriptor &a, const entry_descriptor &b) { return a.semantic_hash < b.semantic_hash; });
    ...
    auto index_bytes = std::span<const std::byte>(reinterpret_cast<const std::byte *>(index.data()), index_size);
    ...
    auto hdr_bytes = std::span<const std::byte>(reinterpret_cast<const std::byte *>(&hdr), 0x38);
...

    {
        ...
        if (!out) {
            return std::unexpected(
                error{error_code::io_error, std::format("failed to open '{}' for writing", temp_path.string())});
        }

        out.write(reinterpret_cast<const char *>(&hdr), sizeof(hdr));

        if (!mphf_blob.empty()) {
            out.write(reinterpret_cast<const char *>(mphf_blob.data()), static_cast<std::streamsize>(mphf_blob.size()));
        }

        out.write(reinterpret_cast<const char *>(index.data()), static_cast<std::streamsize>(index_size));
        ...
        if (!out) {
            return std::unexpected(
                error{error_code::io_error, std::format("write failed for '{}'", temp_path.string())});
        }
    }
